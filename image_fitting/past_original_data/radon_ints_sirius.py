...
with open(prefix+'radon/sirius_radon_ints.csv','wb') as csvfile:
    writer = csv.writer(csvfile)
    row = ['File','Guess x','Guess y','x','y','Rot guess x','Rot guess y','Rot x','Rot y','Verfiied?']
    writer.writerow(row)
    for i,s in enumerate(snames):
        with np.load(s+'.npz') as d:
            ...
            orig_cen = d['orig_cen']
            fname = d['fname']
        row = [fname,h_int[0],h_int[1],orig_cen[0],orig_cen[1],
            guess[0],guess[1],rcen[0],rcen[1]]
        writer.writerow(row)
