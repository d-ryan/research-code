    ...
    for i in inds: #loop over columns
        ...
        if len(nz)>84: #Fit the central 80 pixels (arbitrary)
            ...
            try:
                ...
                px[:,i],c=curve_fit(fitfn,fitcoords,fitvals,p0=p0,sigma=1./np.sqrt(np.abs(fitvals)))
            ...
            try:
                ...
                px[:,i],c=curve_fit(fitfn,nz[2:-2],offset[2:-2],p0=p0,sigma=1./np.sqrt(np.abs(offset[2:-2])))
                check_params(px[:,i],p0)
                #print i,' fit'
            except:
                badsx.append(i)
            ...
            try:
                ...
                px[:,i],c=curve_fit(fitfn,inds,img[:,i],p0=p0,sigma=1./np.sqrt(np.abs(img[:,i])))
                check_params(px[:,i],p0)
                #print i,' fit'
            except:
                badsx.append(i)
    ...
    for i in inds: #loop over columns
        ...
        if len(nz)>84: #Fit the central 80 pixels (arbitrary)
            ...
            try:
                ...
                py[:,i],c=curve_fit(fitfn,fitcoords,fitvals,p0=p0,sigma=1./np.sqrt(np.abs(fitvals)))
            ...
            try:
                ...
                py[:,i],c=curve_fit(fitfn,nz[2:-2],offset[2:-2],p0=p0,sigma=1./np.sqrt(np.abs(offset[2:-2])))
                check_params(py[:,i],p0)
            except:
                badsy.append(i)
        else: #Fit the whole thing; it's probably bad
            try:
                ...
                py[:,i],c=curve_fit(fitfn,inds,img[i,:],p0=p0,sigma=1./np.sqrt(np.abs(img[i,:])))
                check_params(py[:,i],p0)
            except:
                badsy.append(i)
...
if __name__=='__main__':
    for i,f in enumerate(fnames):
        ...
        if split:
            ...
            vert1 = py[1,:]
            vert2 = py[4,:]
            inds = np.arange(len(horiz1))
            good_hs1,good_horiz1,good_vs1,good_vert1 = find_goods(rotated,horiz1,vert1,cenguess,iterations=5)
            
            #will need two sets for each direction
            pars1 = polynomial_fit(good_hs1,good_horiz1,good_vs1,good_vert1)
            plh1,plv1,pph1,ppv1,pch1,pcv1 = pars1
            good_hs2,good_horiz2,good_vs2,good_vert2 = find_goods(rotated,horiz2,vert2,cenguess,iterations=5)
            
            #will need two sets for each direction
            pars2 = polynomial_fit(good_hs2,good_horiz2,good_vs2,good_vert2)
            plh2,plv2,pph2,ppv2,pch2,pcv2 = pars2
            ...
            '''
            for j in range(4):
                if j//2==0: source1 = pars1
                else: source1 = pars2
                if j%2==0: source2 = pars1
                else: source2 = pars2
                for k in range(3):
                    if k==0: intfn=crossline
                    elif k==1: intfn=crossparab
                    else: intfn=crosscubic
                    ints[3*j+k,:] = find_ints(intfn,cenguess,tuple(source1[2*k])+tuple(source2[2*k+1]))
        ...
        if split:
            np.savez_compressed(savename,img=rotated,split=True,orig=img,diff=diff,pad=pad,angle=angle,guess=cenguess,px=px,py=py,
                pars1=pars1,pars2=pars2,ints=ints,good_hs=(good_hs1,good_hs2),good_vs=(good_vs1,good_vs2),good_horiz=(good_horiz1,good_horiz2),
                good_vert=(good_vert1,good_vert2),fname=f)
        else:
