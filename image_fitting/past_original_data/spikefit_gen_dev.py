...
def find_goods(rotated,horiz,vert,cenguess):
    '''
    ...
    if len(good_hs)>150:
        good_hs = good_hs[10:-10]
        good_horiz = good_horiz[10:-10]
    elif len(good_hs)>30:
        good_hs = good_hs[5:-5]
        good_horiz = good_horiz[5:-5]
    if len(good_vs)>150:
        good_vs = good_vs[10:-10]
        good_vert = good_vert[10:-10]
    elif len(good_vs)>30:
        good_vs = good_vs[5:-5]
    ...
    where_small_h = np.where(np.abs(resid_h)<bad_deviation)[0]
    where_small_v = np.where(np.abs(resid_v)<bad_deviation)[0]
...
if __name__=='__main__':
    for i,f in enumerate(fnames):
        ...
        good_hs,good_horiz,good_vs,good_vert = find_goods(rotated,horiz,vert,cenguess)
