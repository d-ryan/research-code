...
if __name__=='__main__':
    for i,f in enumerate(fnames):
        ...
        px,py,bx,by,px2,py2 = fit_both(rotated,cenguess,split)
        ...
        rcen = search_center(rotated,cenguess[0],cenguess[1],rotated.shape[0]/2.,size_cost=10.,theta=[0,90],method=2)
        p_horiz = px2[3,:]
        p_vert = py2[3,:]
        p_inds = np.arange(len(p_horiz))
        
        gp_hs,gp_horiz,gp_vs,gp_vert = find_goods(rotated,p_horiz,p_vert,cenguess,iterations=3)
        
        #Will need two sets for each direction
        good_hs,good_horiz,good_vs,good_vert = find_goods(rotated,horiz,vert,cenguess,iterations=3)
        
        #will need two sets for each direction
        plh,plv,pph,ppv,pch,pcv = polynomial_fit(good_hs,good_horiz,good_vs,good_vert)
        pplh,pplv,ppph,pppv,ppch,ppcv = polynomial_fit(gp_hs,gp_horiz,gp_vs,gp_vert)
        ltup = tuple(plh)+tuple(plv)
        ptup = tuple(pph)+tuple(ppv)
        ctup = tuple(pch)+tuple(pcv)
        pltup = tuple(pplh)+tuple(pplv)
        pptup = tuple(ppph)+tuple(pppv)
        pctup = tuple(ppch)+tuple(ppcv)
        ...
        pints[0,:] = find_ints(crossline,cenguess,pltup)
        pints[1,:] = find_ints(crossparab,cenguess,pptup)
        pints[2,:] = find_ints(crosscubic,cenguess,pctup)
        ...
        np.savez_compressed(savename,img=rotated,orig=img,diff=diff,pad=pad,angle=angle,guess=cenguess,px=px,py=py,plh=plh,plv=plv,
            ...
            good_vert=good_vert,px2=px2,py2=py2,rcen=rcen,gp_hs=gp_hs,gp_horiz=gp_horiz,gp_vs=gp_vs,gp_vert=gp_vert,
            pplh=pplh,pplv=pplv,ppph=ppph,pppv=pppv,ppch=ppch,ppcv=ppcv,pints=pints,fname=f)
