...
def time_plots(times,orbitlist):
    ...
    for i,orb in enumerate(orbitlist):
        star_xy=mk_star_xy(times,[orb])
        star_net=np.sum(star_xy,axis=1)
        xplot=star_net[0,:]/orb.dist*1000.
        yplot=star_net[1,:]/orb.dist*1000.
        ...
        along_a*=1000./orb.dist
        
        if i==len(orbitlist)-1:
            main_orb = ax1.plot(times,along_a)
            plt.setp(main_orb,color='k',linewidth=3.)
            plt.ylim(1.5*np.min(along_a),1.5*np.max(along_a))
        else:
            rand_orb.append(ax1.plot(times,along_a))
            plt.setp(rand_orb[i],color='b',linewidth=2.,alpha=0.1)
    ...
    fig1.savefig('astr_signal_masses.png')
    fig1.savefig('astr_signal_masses.svg')
