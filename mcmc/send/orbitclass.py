...
class orbit(object):
    ...
    def time2true(self,t):
        ...
        return self.mean2true(2.*np.pi*(((t-self.tau)/(self.P*365.2425))%1.))
    def true2time(self,theta):
        '''Given true anomaly, returns absolute time [MJD].
        Restricted to one period after the known epoch of periapsis.'''
        return self.true2mean(theta)/2./np.pi*self.P*365.2425+self.tau
    ...
    def get_plot_data(self,axis='-z',facing=0,npts=10001,tflag=False):
        nus = np.linspace(0.,np.pi*2.,npts)
