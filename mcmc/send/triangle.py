...
def corner(xs, labels=None, extents=None, truths=None, truth_color="#4682b4",
    ...
    K = len(xs)
    factor = 2.          # size of one side of one panel
    ...
    #pl.rcParams['xtick.major.pad']='32'
    ...
    for i, x in enumerate(xs):
        ...
        n, b, p = ax.hist(x, bins=kwargs.get("bins", 50), range=extents[i],
                          histtype="step", color=kwargs.get("color", "k"),linewidth=2)
        ...
        if len(quantiles) > 0:
            xsorted = sorted(x)
            for q in quantiles:
                ax.axvline(xsorted[int(q * len(xsorted))], ls="dotted",
...
def hist2d(x, y, *args, **kwargs):
    ...
    # confidence intervales for a circular gaussian
    # V = 1.0 - np.exp(-0.5 * np.arange(0.5, 2.1, 0.5) ** 2)  

    V = np.array([0.10, 0.5, 0.90])
