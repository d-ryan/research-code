...
def time_plots(times,orbitlist):
    ...
    #ax1.plot(times,xplot,label='East-west')
    plt.xlabel('MJD')
    plt.ylabel('mas')
    plt.title('Beta Pic major axis reflex motion')
    #fig1.savefig('testplotx.png')
    #fig2=plt.figure()
    #ax2=fig2.add_subplot(111)
    #ax1.plot(times,yplot,label='North-south')
    #min=np.min([np.min(yplot),np.min(xplot)])
    #max=np.max([np.max(xplot),np.max(yplot)])
    #plt.ylim(1.5*min,1.5*max)
    #plt.xlabel('MJD')
    #plt.ylabel('mas')
    #plt.title('Reflex motion in y')
    #fig2.savefig('testploty.png')
    if len(orbitlist)==1:
        ...
        #ax1.plot(times,along_a,label='Along projected major axis')
        ax1.plot(times,along_a)
        #if np.max(along_a)>max:
        plt.ylim(1.5*np.min(along_a),1.5*np.max(along_a))
        #plt.xlabel('MJD')
        #plt.ylabel('mas')
        #plt.title('Reflex motion along long axis')
...
if __name__=='__main__':
    ...
    #n_years=30
    #times=make_times(start,start-365.2425*n_years,(n_years*12.)+1.)
    #time_plots(times,get_orbits(filename))



    #Plots xvy
    filename='hr8799_test.csv'
    start=jdcal.gcal2jd(date.today().year,date.today().month,date.today().day)[1]
    n_years=150
